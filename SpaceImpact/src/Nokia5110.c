...
void Nokia5110_PrintChar(unsigned char xpos, unsigned char ypos, unsigned char data, unsigned char is_inverted) {
	...
	ptr[1] = height;
	for (i=0; i<width; i++) {
		ptr[2+i] = ASCII[data - 0x20][i];
	}
	Nokia5110_PrintBMP_BW(xpos, ypos, ptr, is_inverted);
}
...
void Nokia5110_PrintString(unsigned char xpos, unsigned char ypos, char *ptr, unsigned char is_inverted) {
	unsigned char i;
	for (i=0; *ptr; i++, ptr++) {
		Nokia5110_PrintChar(xpos + 6*i, ypos, (unsigned char)*ptr, is_inverted);
	}
}
...
//********Nokia5110_PrintFullImage*****************
// Fill the whole screen buffer by copying a 48x84 bitmap image.
// inputs: ptr  pointer to 504 byte bitmap
// outputs: none
// assumes: LCD is in default horizontal addressing mode (V = 0)
void Nokia5110_PrintFullImage(const char *ptr, const unsigned char is_inverted, const unsigned char is_overlay){
	int i;
	for(i=0; i<(MAX_X*MAX_Y/8); i=i+1){
		if (!is_inverted) {
			if (is_overlay) {
...
// outputs: none
void Nokia5110_PrintBMP(unsigned char xpos, unsigned char ypos, const unsigned char *ptr, unsigned char threshold, const unsigned char is_inverted, const unsigned char is_overlay){
...
void Nokia5110_PrintBMP_BW(signed char xpos, signed char ypos, const unsigned char *ptr, const unsigned char is_inverted) {
	unsigned char width  = ptr[0];
	unsigned char height = ptr[1];
	unsigned short i;
	unsigned short screen_x;  // 504 bytes
	signed char screen_y;     // 0 - 5
	signed char screen_y_rem; // 0 - 7
	signed char x;
	signed char y;
	unsigned char px_byte;
	for (i=0; i<width*(height/8 + ((height % 8) != 0)); i++) {
		x = xpos + (i % width);
		y = ypos + (i / width);
		px_byte = ptr[2 + i];
		if (x >= 0 && x < SCREENW && y >= 1 - height && y < SCREENH) { 
			screen_y_rem = y % 8;
			screen_y = y / 8;
			screen_x = x + SCREENW*screen_y;
			if (screen_y_rem > 0) {
				if (!is_inverted) {
					Screen[screen_x] |= px_byte << screen_y_rem;
				} else {
					Screen[screen_x] &= ~(px_byte << screen_y_rem);
				}
				if (screen_y < 5) {
					if (!is_inverted) {
						Screen[screen_x + SCREENW] |= px_byte >> (8 - screen_y_rem);
					} else {
						Screen[screen_x + SCREENW] &= ~(px_byte >> (8 - screen_y_rem));
					}
				}
				...
				if (screen_y_rem < 0) {
					if (!is_inverted) {
						Screen[screen_x] |= px_byte >> (8 - (8 + screen_y_rem));
					} else {
						Screen[screen_x] &= ~(px_byte >> (8 - (8 + screen_y_rem)));
					}
				} else {
					if (!is_inverted) {
						Screen[screen_x] |= px_byte;
					} else {
						Screen[screen_x] &= ~(px_byte);
					}
